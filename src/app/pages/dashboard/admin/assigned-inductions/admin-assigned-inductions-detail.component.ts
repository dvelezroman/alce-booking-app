...
@Component({
  selector: 'app-admin-assigned-inductions-detail',
  standalone: true,
  imports: [CommonModule, RouterModule, ReactiveFormsModule, ModalComponent],
  templateUrl: './admin-assigned-inductions-detail.component.html',
  styleUrl: './admin-assigned-inductions-detail.component.scss',
})
export class AdminAssignedInductionsDetailComponent implements OnInit, OnDestroy {
    ...
    this.route.paramMap
      .pipe(
        ...
        switchMap((params) => {
          ...
          this.error = null;
          this.row = null;
          return this.leadScheduling.getAssignedInduction(id);
        }),
      )
      .subscribe({
        next: (data) => {
  ...
  submit(): void {
    ...
    this.leadScheduling.submitAdminInductionReport(this.row.id, body)
      .subscribe({
        next: () => {
